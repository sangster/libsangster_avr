...
SA_FUNC void ring_buff_8_reset(RingBuff8* buff)
{
    buff->head = 0;
    buff->tail = 0;
    buff->used = 0;
...
SA_FUNC bool ring_buff_8_is_empty(const RingBuff8* buff)
{
    return buff->used == 0;
}


SA_FUNC bool ring_buff_8_is_full(const RingBuff8* buff)
{
    return buff->used == buff->size;
}


SA_FUNC uint8_t ring_buff_8_avg(RingBuff8* buff)
{
    uint16_t sum = 0;
